...
class FileCollection:
    ...
    def _set_filetype(self):
        ...
        elif self._ext in ["ores", "states", "staples", "staplestates"]:
            self._num_header_lines = 0
    ...
    def _open_thread_files(self):
        for thread in range(self._num_threads):
            filename = "{}/{}-{}.{}".format(
                self._fileinfo.inputdir, self._fileinfo.filebase, thread, self._ext
            )
    ...
    def _open_replica_files(self):
        for params in self._all_exchange_params:
            filename = "{}/{}-{}.{}".format(
                self._fileinfo.outputdir, self._fileinfo.filebase, params, self._ext
            )
            self._replica_files.append(open(filename, "w"))

    def _write_headers(self):
        for thread_f, replica_f in zip(self._thread_files, self._replica_files):
