...
def main():
    ...
    out_filebase = "{}_{}-barrier-vs-temp".format(args.output_filebase, args.tag)
    ...
    for system, vari in zip(args.systems, args.varis):
        filebase = "{}/{}-{}_{}-lfes".format(args.input_dir, system, vari, args.tag)
        ...
        upper_stds = []
        for temp in lfes:
            peaks, stds = calc_barrier_heights_and_errors(
                lfes[temp], lfe_stds[temp], temp
            )
...
def parse_args():
    ...
    parser.add_argument("output_filebase", type=str, help="Output filebase")
