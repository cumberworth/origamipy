...
def main():
    args = parse_args()
    f = setup_figure()
    gs = gridspec.GridSpec(1, 2, f, width_ratios=[10, 1], height_ratios=[1])
...
def plot_figure(f, ax, args):
    ...
    for i, tag in enumerate(tags):
        ...
        darkcolor = plotutils.darken_color(color[:3], 0.4)
        if contin:
            ax.fill_between(
                xvars, aves[tag] + stds[tag], aves[tag] - stds[tag], color="0.8"
            )
        else:
            ax.errorbar(
                xvars,
...
def parse_args():
    ...
    parser.add_argument(
        "--xtag", default="temp", type=str, help="Dependent variable tag"
    )
    parser.add_argument("--rtag", type=str, help="Tag to slice on")
    parser.add_argument("--rvalue", type=float, help="Slice value")
    parser.add_argument(
        "--continuous", default=False, type=bool, help="Plot curves as continuous"
    )
