...
def main():
    ...
    for tag1 in tag1s:
        inp_filebase = "{}/{}-{}_{}-{}-lfes".format(
            args.input_dir, args.system, args.vari, tag1, tag2
        )
        ...
        gs_main = gridspec.GridSpecFromSubplotSpec(
            1, 1, subplot_spec=gs[0, :], wspace=0, hspace=0
        )
        ...
        cbar = mpl.colorbar.ColorbarBase(
            ax, orientation="horizontal", cmap=cmap, norm=norm
        )
...
def parse_args():
    ...
    parser.add_argument("tag2", type=str, help="Cumaltive OP tag to plot against")
    parser.add_argument("--temps", nargs="+", type=float, help="Temperatures")
