...
def main():
    ...
    out_filebase = "{}/{}_run-{}-{}".format(
        args.output_dir, args.filebase, decor_outs.start_run, decor_outs.end_run
    )
    ...
    print("Guess temperature: {:.3f} K".format(np.around(halfway_temp, decimals=3)))
    conds = conditions.SimConditions(
        {"temp": args.guess_temp, "staple_m": args.staple_m, "bias": biases.NoBias()},
        fileformatter,
        staple_lengths,
    )
    bias = biases.NoBias()
