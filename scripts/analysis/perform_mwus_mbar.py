...
def main():
    ...
    for rep in range(args.reps):
        rep_sim_collections = outputs.create_sim_collections(
            inp_filebase, reps_all_conditions[rep], rep, args.start_run, args.end_run
        )
    ...
    decor_outs = decorrelate.DecorrelatedOutputs(
        sim_collections, all_conditions=all_conditions, rep_conditions_equal=False
    )
    ...
    out_filebase = "{}/{}_run-{}-{}_iter-{}".format(
        args.analysis_dir, args.filebase, args.start_run, args.end_run, args.itr
    )
    ...
    for temp in np.linspace(melting_temp - 5, melting_temp + 5, 100):
        conds = conditions.SimConditions(
            {"temp": temp, "staple_m": args.staple_m, "bias": biases.NoBias()},
            fileformatter,
            staple_lengths,
        )
    ...
    aves, stds = calc_reduced_expectations(conds, mbarws, all_decor_outs, se_tags)
...
def est_melting_temp_and_barrier(
    ...
    conds = conditions.SimConditions(
        {"temp": melting_temp, "staple_m": args.staple_m, "bias": bias},
        fileformatter,
        staple_lengths,
    )
...
def parse_args():
    ...
    parser.add_argument("start_run", type=int, help="Run to concatenate from")
    parser.add_argument("end_run", type=int, help="Run to concatenate to")
