...
def main():
    ...
    guess_temps = np.linspace(old_temps.iloc[0], old_temps.iloc[-1], num=num_temps)
    desired_ops = np.linspace(
        args.max_op - args.max_op * args.op_margin,
        args.max_op * args.op_margin,
        num=num_temps,
    )
    new_temps = minimize(
